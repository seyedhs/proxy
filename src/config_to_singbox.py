...
class ConfigToSingbox:
    ...
    def _make_unique_tag(self, base_tag: str, used_tags: set, index: int, protocol_type: str) -> str:
        """Ensure tag is unique; if duplicate, append index to make it unique."""
        tag = base_tag
        if tag in used_tags:
            tag = f"{base_tag} #{index}"
        # If still duplicate (very unlikely), keep appending until unique
        counter = 2
        while tag in used_tags:
            tag = f"{base_tag} #{index}-{counter}"
            counter += 1
        return tag

    def convert_to_singbox(self, config: str, index: int, protocol_type: str, used_tags: set) -> Optional[Dict]:
        try:
            ...
            if config_lower.startswith('vmess://'):
                ...
                base_tag = data.get('name') or f"{protocol_type} {index} - {data['add']}:{data['port']}"
                tag = self._make_unique_tag(base_tag, used_tags, index, protocol_type)
                ...
                if not data: return None
                base_tag = data.get('name') or f"{protocol_type} {index} - {data['address']}:{data['port']}"
                tag = self._make_unique_tag(base_tag, used_tags, index, protocol_type)
                transport, tls = transport_builder.build_singbox_settings(data)
                outbound = {
    ...
    def process_configs(self):
        ...
        for config in configs:
            ...
            if protocol_name:
                converted = self.convert_to_singbox(config, counters[protocol_name], protocol_name, used_tags)
                if converted:
                    used_tags.add(converted['tag'])
        ...
        final_config = {
            ...
            "dns": {
                "servers": [
                    {"type": "https", "server": "8.8.8.8", "detour": "زن زندگی آزادی", "tag": "dns-remote"},
            ...
            "outbounds": [
                {"type": "selector", "tag": "زن زندگی آزادی", "outbounds": ["بهترین پینگ"] + valid_tags + ["direct"]},
                {"type": "direct", "tag": "direct"},
                {"type": "urltest", "tag": "بهترین پینگ", "outbounds": valid_tags, "url": "https://www.gstatic.com/generate_204", "interrupt_exist_connections": False, "interval": "30s"}
            ] + outbounds,
            "route": {
                "rules": [
