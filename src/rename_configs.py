...
def to_persian_number(n: int) -> str:
...
class ConfigRenamer:
    def __init__(self, location_file: str):
        ...
        # A single running counter across the whole file, independent of
        # flag/country. This should be the LAST step in the pipeline (after
        # testing/truncating to the final top-N and after location
        # enrichment) so the numbering reflects the final published order --
        # e.g. #1 is the first (fastest) config in the file, #N is the last.
        self.counter = 0

    def build_display_name(self, flag: str) -> str:
        self.counter += 1
        return f"{to_persian_number(self.counter)} - زن زندگی آزادی {flag}"
    ...
    def rename_config(self, config: str, index: int, protocol_type: str) -> Optional[str]:
        try:
            ...
            if config_lower.startswith('vmess://'):
                ...
                new_name = self.build_display_name(flag)
                data['ps'] = new_name
