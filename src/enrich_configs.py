...
class ConfigEnricher:
    ...
    def process_configs(self, input_file: str, output_file: str):
        ...
        completed = 0
        max_workers = min(20, total) or 1
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(self.get_location, address): address for address in unique_addresses}
            for future in as_completed(futures):
                completed += 1
                try:
                    future.result()
                except Exception as e:
                    logger.debug(f"Location lookup failed for {futures[future]}: {e}")
