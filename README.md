# Proxy Configs

<!-- dynamic timestamp placeholders for workflow sed script -->
channel_stats_chart.svg?v=1789992117
performance_report.html?v=1789992117

### Performance Chart
![Performance Chart](assets/channel_stats_chart.svg?v=1789992117)

### Performance Report
[View Report](assets/performance_report.html?v=1789992117)

## HTTP

<!-- STATS:HTTP:START -->

_Last updated: 2026-09-21 06:16 UTC_

```mermaid
xychart-beta
    title "Healthy HTTP proxies per source"
    x-axis ["ProxyScrape", "proxifly/free-proxy-list", "Previous scan (re-checked)", "TheSpeedX/PROXY-List", "ALIILAPRO/Proxy", "monosans/proxy-list"]
    y-axis "Healthy proxies"
    bar [55, 53, 38, 37, 35, 28]
```

| Source | Healthy proxies |
|---|---|
| ProxyScrape | 55 |
| proxifly/free-proxy-list | 53 |
| Previous scan (re-checked) | 38 |
| TheSpeedX/PROXY-List | 37 |
| ALIILAPRO/Proxy | 35 |
| monosans/proxy-list | 28 |
| **Total unique** | **246** |

<!-- STATS:HTTP:END -->

## SOCKS5

<!-- STATS:SOCKS5:START -->

_Last updated: 2026-09-21 06:16 UTC_

```mermaid
xychart-beta
    title "Healthy SOCKS5 proxies per source"
    x-axis ["proxifly/free-proxy-list", "ProxyScrape", "TheSpeedX/PROXY-List", "Previous scan (re-checked)", "ALIILAPRO/Proxy", "monosans/proxy-list"]
    y-axis "Healthy proxies"
    bar [92, 71, 56, 45, 37, 19]
```

| Source | Healthy proxies |
|---|---|
| proxifly/free-proxy-list | 92 |
| ProxyScrape | 71 |
| TheSpeedX/PROXY-List | 56 |
| Previous scan (re-checked) | 45 |
| ALIILAPRO/Proxy | 37 |
| monosans/proxy-list | 19 |
| **Total unique** | **320** |

<!-- STATS:SOCKS5:END -->
