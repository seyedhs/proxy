# Proxy Configs

<!-- dynamic timestamp placeholders for workflow sed script -->
channel_stats_chart.svg?v=1790424082
performance_report.html?v=1790424082

### Performance Chart
![Performance Chart](assets/channel_stats_chart.svg?v=1790424082)

### Performance Report
[View Report](assets/performance_report.html?v=1790424082)

## HTTP

<!-- STATS:HTTP:START -->

_Last updated: 2026-09-26 06:16 UTC_

```mermaid
xychart-beta
    title "Healthy HTTP proxies per source"
    x-axis ["proxifly/free-proxy-list", "Previous scan (re-checked)", "TheSpeedX/PROXY-List", "ProxyScrape", "ALIILAPRO/Proxy", "monosans/proxy-list"]
    y-axis "Healthy proxies"
    bar [67, 45, 35, 25, 17, 15]
```

| Source | Healthy proxies |
|---|---|
| proxifly/free-proxy-list | 67 |
| Previous scan (re-checked) | 45 |
| TheSpeedX/PROXY-List | 35 |
| ProxyScrape | 25 |
| ALIILAPRO/Proxy | 17 |
| monosans/proxy-list | 15 |
| **Total unique** | **204** |

<!-- STATS:HTTP:END -->

## SOCKS5

<!-- STATS:SOCKS5:START -->

_Last updated: 2026-09-26 06:16 UTC_

```mermaid
xychart-beta
    title "Healthy SOCKS5 proxies per source"
    x-axis ["proxifly/free-proxy-list", "ProxyScrape", "TheSpeedX/PROXY-List", "Previous scan (re-checked)", "ALIILAPRO/Proxy", "monosans/proxy-list"]
    y-axis "Healthy proxies"
    bar [96, 46, 38, 24, 23, 14]
```

| Source | Healthy proxies |
|---|---|
| proxifly/free-proxy-list | 96 |
| ProxyScrape | 46 |
| TheSpeedX/PROXY-List | 38 |
| Previous scan (re-checked) | 24 |
| ALIILAPRO/Proxy | 23 |
| monosans/proxy-list | 14 |
| **Total unique** | **241** |

<!-- STATS:SOCKS5:END -->
