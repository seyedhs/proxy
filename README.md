# Proxy Configs

<!-- dynamic timestamp placeholders for workflow sed script -->
channel_stats_chart.svg?v=1788112890
performance_report.html?v=1788112890

### Performance Chart
![Performance Chart](assets/channel_stats_chart.svg?v=1788112890)

### Performance Report
[View Report](assets/performance_report.html?v=1788112890)

## HTTP

<!-- STATS:HTTP:START -->

_Last updated: 2026-08-30 16:04 UTC_

```mermaid
xychart-beta
    title "Healthy HTTP proxies per source"
    x-axis ["monosans/proxy-list", "Previous scan (re-checked)", "TheSpeedX/PROXY-List", "ProxyScrape", "ALIILAPRO/Proxy", "proxifly/free-proxy-list"]
    y-axis "Healthy proxies"
    bar [76, 43, 39, 36, 19, 9]
```

| Source | Healthy proxies |
|---|---|
| monosans/proxy-list | 76 |
| Previous scan (re-checked) | 43 |
| TheSpeedX/PROXY-List | 39 |
| ProxyScrape | 36 |
| ALIILAPRO/Proxy | 19 |
| proxifly/free-proxy-list | 9 |
| **Total unique** | **222** |

<!-- STATS:HTTP:END -->

## SOCKS5

<!-- STATS:SOCKS5:START -->

_Last updated: 2026-08-30 16:04 UTC_

```mermaid
xychart-beta
    title "Healthy SOCKS5 proxies per source"
    x-axis ["ProxyScrape", "TheSpeedX/PROXY-List", "ALIILAPRO/Proxy", "Previous scan (re-checked)", "monosans/proxy-list", "proxifly/free-proxy-list"]
    y-axis "Healthy proxies"
    bar [58, 54, 35, 29, 24, 23]
```

| Source | Healthy proxies |
|---|---|
| ProxyScrape | 58 |
| TheSpeedX/PROXY-List | 54 |
| ALIILAPRO/Proxy | 35 |
| Previous scan (re-checked) | 29 |
| monosans/proxy-list | 24 |
| proxifly/free-proxy-list | 23 |
| **Total unique** | **223** |

<!-- STATS:SOCKS5:END -->
