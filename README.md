# Proxy Configs

<!-- dynamic timestamp placeholders for workflow sed script -->
channel_stats_chart.svg?v=1789646615
performance_report.html?v=1789646615

### Performance Chart
![Performance Chart](assets/channel_stats_chart.svg?v=1789646615)

### Performance Report
[View Report](assets/performance_report.html?v=1789646615)

## HTTP

<!-- STATS:HTTP:START -->

_Last updated: 2026-09-17 06:13 UTC_

```mermaid
xychart-beta
    title "Healthy HTTP proxies per source"
    x-axis ["proxifly/free-proxy-list", "Previous scan (re-checked)", "monosans/proxy-list", "TheSpeedX/PROXY-List", "ProxyScrape", "ALIILAPRO/Proxy", "roosterkid/openproxylist"]
    y-axis "Healthy proxies"
    bar [66, 50, 40, 34, 19, 13, 1]
```

| Source | Healthy proxies |
|---|---|
| proxifly/free-proxy-list | 66 |
| Previous scan (re-checked) | 50 |
| monosans/proxy-list | 40 |
| TheSpeedX/PROXY-List | 34 |
| ProxyScrape | 19 |
| ALIILAPRO/Proxy | 13 |
| roosterkid/openproxylist | 1 |
| **Total unique** | **223** |

<!-- STATS:HTTP:END -->

## SOCKS5

<!-- STATS:SOCKS5:START -->

_Last updated: 2026-09-17 06:13 UTC_

```mermaid
xychart-beta
    title "Healthy SOCKS5 proxies per source"
    x-axis ["proxifly/free-proxy-list", "ProxyScrape", "TheSpeedX/PROXY-List", "ALIILAPRO/Proxy", "Previous scan (re-checked)", "monosans/proxy-list", "roosterkid/openproxylist"]
    y-axis "Healthy proxies"
    bar [91, 67, 40, 32, 27, 13, 1]
```

| Source | Healthy proxies |
|---|---|
| proxifly/free-proxy-list | 91 |
| ProxyScrape | 67 |
| TheSpeedX/PROXY-List | 40 |
| ALIILAPRO/Proxy | 32 |
| Previous scan (re-checked) | 27 |
| monosans/proxy-list | 13 |
| roosterkid/openproxylist | 1 |
| **Total unique** | **271** |

<!-- STATS:SOCKS5:END -->
