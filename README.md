# Proxy Configs

<!-- dynamic timestamp placeholders for workflow sed script -->
channel_stats_chart.svg?v=1788199341
performance_report.html?v=1788199341

### Performance Chart
![Performance Chart](assets/channel_stats_chart.svg?v=1788199341)

### Performance Report
[View Report](assets/performance_report.html?v=1788199341)

## HTTP

<!-- STATS:HTTP:START -->

_Last updated: 2026-08-31 12:07 UTC_

```mermaid
xychart-beta
    title "Healthy HTTP proxies per source"
    x-axis ["monosans/proxy-list", "Previous scan (re-checked)", "TheSpeedX/PROXY-List", "ProxyScrape", "ALIILAPRO/Proxy", "proxifly/free-proxy-list"]
    y-axis "Healthy proxies"
    bar [71, 48, 39, 26, 24, 8]
```

| Source | Healthy proxies |
|---|---|
| monosans/proxy-list | 71 |
| Previous scan (re-checked) | 48 |
| TheSpeedX/PROXY-List | 39 |
| ProxyScrape | 26 |
| ALIILAPRO/Proxy | 24 |
| proxifly/free-proxy-list | 8 |
| **Total unique** | **216** |

<!-- STATS:HTTP:END -->

## SOCKS5

<!-- STATS:SOCKS5:START -->

_Last updated: 2026-08-31 12:07 UTC_

```mermaid
xychart-beta
    title "Healthy SOCKS5 proxies per source"
    x-axis ["ProxyScrape", "TheSpeedX/PROXY-List", "Previous scan (re-checked)", "monosans/proxy-list", "proxifly/free-proxy-list", "ALIILAPRO/Proxy", "roosterkid/openproxylist"]
    y-axis "Healthy proxies"
    bar [62, 51, 35, 35, 31, 27, 1]
```

| Source | Healthy proxies |
|---|---|
| ProxyScrape | 62 |
| TheSpeedX/PROXY-List | 51 |
| Previous scan (re-checked) | 35 |
| monosans/proxy-list | 35 |
| proxifly/free-proxy-list | 31 |
| ALIILAPRO/Proxy | 27 |
| roosterkid/openproxylist | 1 |
| **Total unique** | **242** |

<!-- STATS:SOCKS5:END -->
