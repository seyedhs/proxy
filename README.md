# Proxy Configs

<!-- dynamic timestamp placeholders for workflow sed script -->
channel_stats_chart.svg?v=1788220958
performance_report.html?v=1788220958

### Performance Chart
![Performance Chart](assets/channel_stats_chart.svg?v=1788220958)

### Performance Report
[View Report](assets/performance_report.html?v=1788220958)

## HTTP

<!-- STATS:HTTP:START -->

_Last updated: 2026-08-31 18:08 UTC_

```mermaid
xychart-beta
    title "Healthy HTTP proxies per source"
    x-axis ["monosans/proxy-list", "Previous scan (re-checked)", "TheSpeedX/PROXY-List", "ProxyScrape", "ALIILAPRO/Proxy", "proxifly/free-proxy-list"]
    y-axis "Healthy proxies"
    bar [66, 51, 44, 33, 23, 11]
```

| Source | Healthy proxies |
|---|---|
| monosans/proxy-list | 66 |
| Previous scan (re-checked) | 51 |
| TheSpeedX/PROXY-List | 44 |
| ProxyScrape | 33 |
| ALIILAPRO/Proxy | 23 |
| proxifly/free-proxy-list | 11 |
| **Total unique** | **228** |

<!-- STATS:HTTP:END -->

## SOCKS5

<!-- STATS:SOCKS5:START -->

_Last updated: 2026-08-31 18:08 UTC_

```mermaid
xychart-beta
    title "Healthy SOCKS5 proxies per source"
    x-axis ["ProxyScrape", "TheSpeedX/PROXY-List", "proxifly/free-proxy-list", "Previous scan (re-checked)", "monosans/proxy-list", "ALIILAPRO/Proxy", "roosterkid/openproxylist"]
    y-axis "Healthy proxies"
    bar [59, 55, 39, 37, 33, 23, 1]
```

| Source | Healthy proxies |
|---|---|
| ProxyScrape | 59 |
| TheSpeedX/PROXY-List | 55 |
| proxifly/free-proxy-list | 39 |
| Previous scan (re-checked) | 37 |
| monosans/proxy-list | 33 |
| ALIILAPRO/Proxy | 23 |
| roosterkid/openproxylist | 1 |
| **Total unique** | **247** |

<!-- STATS:SOCKS5:END -->
