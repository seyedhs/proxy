# Proxy Configs

<!-- dynamic timestamp placeholders for workflow sed script -->
channel_stats_chart.svg?v=1788030873
performance_report.html?v=1788030873

### Performance Chart
![Performance Chart](assets/channel_stats_chart.svg?v=1788030873)

### Performance Report
[View Report](assets/performance_report.html?v=1788030873)

## HTTP

<!-- STATS:HTTP:START -->

_Last updated: 2026-09-26 12:18 UTC_

```mermaid
xychart-beta
    title "Healthy HTTP proxies per source"
    x-axis ["proxifly/free-proxy-list", "Previous scan (re-checked)", "TheSpeedX/PROXY-List", "ALIILAPRO/Proxy", "ProxyScrape", "monosans/proxy-list", "roosterkid/openproxylist"]
    y-axis "Healthy proxies"
    bar [68, 53, 45, 25, 21, 14, 1]
```

| Source | Healthy proxies |
|---|---|
| proxifly/free-proxy-list | 68 |
| Previous scan (re-checked) | 53 |
| TheSpeedX/PROXY-List | 45 |
| ALIILAPRO/Proxy | 25 |
| ProxyScrape | 21 |
| monosans/proxy-list | 14 |
| roosterkid/openproxylist | 1 |
| **Total unique** | **227** |

<!-- STATS:HTTP:END -->

## SOCKS5

<!-- STATS:SOCKS5:START -->

_Last updated: 2026-09-26 12:18 UTC_

```mermaid
xychart-beta
    title "Healthy SOCKS5 proxies per source"
    x-axis ["proxifly/free-proxy-list", "TheSpeedX/PROXY-List", "ProxyScrape", "ALIILAPRO/Proxy", "Previous scan (re-checked)", "monosans/proxy-list"]
    y-axis "Healthy proxies"
    bar [97, 51, 41, 25, 21, 16]
```

| Source | Healthy proxies |
|---|---|
| proxifly/free-proxy-list | 97 |
| TheSpeedX/PROXY-List | 51 |
| ProxyScrape | 41 |
| ALIILAPRO/Proxy | 25 |
| Previous scan (re-checked) | 21 |
| monosans/proxy-list | 16 |
| **Total unique** | **251** |

<!-- STATS:SOCKS5:END -->
