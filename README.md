# Proxy Configs

<!-- dynamic timestamp placeholders for workflow sed script -->
channel_stats_chart.svg?v=1790186497
performance_report.html?v=1790186497

### Performance Chart
![Performance Chart](assets/channel_stats_chart.svg?v=1790186497)

### Performance Report
[View Report](assets/performance_report.html?v=1790186497)

## HTTP

<!-- STATS:HTTP:START -->

_Last updated: 2026-09-23 12:14 UTC_

```mermaid
xychart-beta
    title "Healthy HTTP proxies per source"
    x-axis ["proxifly/free-proxy-list", "TheSpeedX/PROXY-List", "Previous scan (re-checked)", "monosans/proxy-list", "ALIILAPRO/Proxy", "ProxyScrape", "roosterkid/openproxylist"]
    y-axis "Healthy proxies"
    bar [77, 45, 42, 31, 26, 16, 2]
```

| Source | Healthy proxies |
|---|---|
| proxifly/free-proxy-list | 77 |
| TheSpeedX/PROXY-List | 45 |
| Previous scan (re-checked) | 42 |
| monosans/proxy-list | 31 |
| ALIILAPRO/Proxy | 26 |
| ProxyScrape | 16 |
| roosterkid/openproxylist | 2 |
| **Total unique** | **239** |

<!-- STATS:HTTP:END -->

## SOCKS5

<!-- STATS:SOCKS5:START -->

_Last updated: 2026-09-23 12:14 UTC_

```mermaid
xychart-beta
    title "Healthy SOCKS5 proxies per source"
    x-axis ["proxifly/free-proxy-list", "ProxyScrape", "TheSpeedX/PROXY-List", "Previous scan (re-checked)", "ALIILAPRO/Proxy", "monosans/proxy-list"]
    y-axis "Healthy proxies"
    bar [97, 62, 62, 37, 33, 11]
```

| Source | Healthy proxies |
|---|---|
| proxifly/free-proxy-list | 97 |
| ProxyScrape | 62 |
| TheSpeedX/PROXY-List | 62 |
| Previous scan (re-checked) | 37 |
| ALIILAPRO/Proxy | 33 |
| monosans/proxy-list | 11 |
| **Total unique** | **302** |

<!-- STATS:SOCKS5:END -->
